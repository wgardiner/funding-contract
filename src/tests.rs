...
mod tests {
    ...
    use cosmwasm_std::{coins, from_binary, Api, Extern, HumanAddr, StdError};
    ...
    fn mock_proposal(mut deps: &mut Extern<MockStorage, MockApi, MockQuerier>, msg: HandleMsg) {
        let env = mock_env("proposer_0", &coins(1000, "earth"));
    ...
    fn fails_create_proposal_invalid_address() {
        ...
        let env = mock_env("any_user", &coins(1000, "earth"));
        let res = handle(&mut deps, env, proposal_msg);
        match res {
    ...
    fn create_proposal_no_proposer_list() {
        ...
        // try to create a proposal as "any_user"
        let env = mock_env("any_user", &coins(1000, "earth"));
        let res = handle(&mut deps, env, proposal_msg).unwrap();
    ...
    fn fails_create_vote_invalid_address() {
        ...
        let mut env = mock_env("any_user", &coins(1000, "earth"));
        env.block.time = env.block.time + 86400 * 3;

        // send message.
        let res = handle(&mut deps, env, vote_msg);
        match res {
            Err(StdError::Unauthorized { .. }) => {}
            _ => panic!("Must return error"),
        }
    ...
    fn fails_create_vote_invalid_proposal() {
        ...
        let mut env = mock_env("voter_0", &coins(1000, "earth"));
    ...
    fn create_vote_no_voter_list() {
        ...
        // send message.
        let _res = handle(&mut deps, env, vote_msg).unwrap();
    ...
    fn create_vote() {
        ...
        let res = query(
            &mut deps,
            //mock_env("voter_0", &coins(1000, "earth")),
    ...
    fn unique_votes() {
        ...
        let votes_by_voter_0: Vec<_> = pretty_result
            .iter()
            .filter(|x| x.1 == HumanAddr("voter_0".to_string()))
            .collect();
        assert_eq!(votes_by_voter_0.len(), 2);
        let votes_by_voter_1: Vec<_> = pretty_result
            .iter()
            .filter(|x| x.1 == HumanAddr("voter_1".to_string()))
            .collect();
        ...
        let votes_for_prop_0_by_voter_0: Vec<_> = pretty_result
            .iter()
            .filter(|x| x.1 == HumanAddr("voter_1".to_string()) && x.0 == 0)
            .collect();
    ...
    fn fail_distribute_funds_invalid_period() {
        ...
        let mut env = mock_env("owner", &coins(1000, "earth"));
